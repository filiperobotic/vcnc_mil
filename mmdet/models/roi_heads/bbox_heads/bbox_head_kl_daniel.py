...
class BBoxHeadKL(BaseModule):
            ...
            if pos_inds.any():
                ...
                if self.reg_class_agnostic:
                    ...
                    pos_bbox_var = bbox_var.view(
                        bbox_var.size(0), -1)[pos_inds.type(torch.bool)]
                else:
                    pos_bbox_pred = bbox_pred.view(
                    ...
                    pos_bbox_var = bbox_var.view(
                        bbox_var.size(0), self.num_classes,
                        -1)[pos_inds.type(torch.bool),
                            labels[pos_inds.type(torch.bool)]]
                ...
                losses['loss_bbox'] = bbox_pred[pos_inds].sum() * 0
    ...
    def _predict_by_feat_single(
        ...
        if rcnn_test_cfg is None:
            ...
            var_det, var_labels = self.kl_nms(
                bboxes,
                scores,
                rcnn_test_cfg.score_thr, 
                rcnn_test_cfg.nms,
                rcnn_test_cfg.max_per_img,
                variance)
            results.bboxes = var_det[:, :-1]
            results.scores = var_det[:, -1]
            results.labels = var_labels
    ...
    def kl_nms(self, bboxes, scores, score_thres, nms_cfg, max_num=-1, variance=None):
        ...
        if len(output_boxes) == 0:
            ...
            output_boxes, output_scores, output_labels = torch.cat(output_boxes).to(torch.device("cpu")), torch.cat(output_scores).to(torch.device("cpu")), torch.cat(output_labels).to(torch.device("cpu"))

            # sort prediction
            sort_inds = torch.argsort(output_scores, descending=True).to(torch.device("cpu"))
